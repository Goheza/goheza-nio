...
export async function createCampaign(input: CreateCampaignInput): Promise<Campaign> {
  ...
  const payload = {
    ...
    // Requires the live_duration_days migration. submission_deadline /
    // live_starts_at / live_ends_at are intentionally left null here —
    // approveCampaign() in lib/admin-campaigns.ts sets them once the
    // campaign actually goes live, using this value.
...
export async function getCampaignWithStats(id: string): Promise<CampaignSummary | null> {
  ...
  return {
    ...
    countries: campaign.target_countries && campaign.target_countries.length > 0 ? campaign.target_countries : 'global',
    views,
