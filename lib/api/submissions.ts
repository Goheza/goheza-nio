...
import { deleteSubmissionVideo } from './storage'
...
export async function approveSubmission(submissionId: string, campaignId: string): Promise<void> {
    ...
    const [{ data: campaign, error: campaignError }, { data: existingApproved, error: countError }] = await Promise.all(
        [
            supabase
                .from('campaigns')
...
export async function rejectSubmission(submissionId: string, feedback: string): Promise<void> {
    ...
    const { data: submission, error: fetchError } = await supabase
        .from('campaign_submissions')
        .select('video_path, video_bucket')
        .eq('id', submissionId)
        .single()
    ...
    if (submission?.video_path && submission?.video_bucket) {
        try {
            await supabase.storage.from(submission.video_bucket).remove([submission.video_path])
        } catch (e) {
            console.error(`Failed to delete video for rejected submission ${submissionId}:`, e)
        }
    }
}
