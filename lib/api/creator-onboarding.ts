...
export async function getCreatorSocialAccounts(userId: string): Promise<string[]> {
    const { data, error } = await supabase.from('creator_social_accounts').select('platform').eq('user_id', userId)

    if (error) throw error
    return (data ?? []).map((r) => r.platform)
}

export async function submitCreatorOnboarding(input: CreatorOnboardingInput): Promise<CreatorProfile> {
    const hasPaymentDetails =
        input.paymentMethod === 'bank'
            ? !!(input.bankAccountName && input.bankAccountNumber)
            : input.paymentMethod === 'mobile'
            ? !!(input.mobileName && input.mobilePhone)
            : false
    ...
    const payload = {
        ...
        payment_account_name: input.paymentMethod === 'bank' ? input.bankAccountName : input.mobileName || null,
...
export function isCreatorOnboardingComplete(
    profile: Pick<CreatorProfile, 'display_name' | 'username' | 'country' | 'has_payment_details'> | null
): boolean {
    return !!(profile?.display_name && profile?.username && profile?.country && profile?.has_payment_details)
}
